    ...
    public function createTicket(TicketCreateDto $ticketData): ApiResponseDto
    {
        try {
            ...
            if($files == null){
                ...
                $ticketObject = json_encode($ticketArray, JSON_UNESCAPED_UNICODE);
                // Desk365 curl samples use -F ticket_object=... (multipart), not query string.
                // Putting JSON in the URL causes HTTP 414 on long email bodies.
                $endpoint = $this->getEndpoint('tickets/create_with_attachment');
